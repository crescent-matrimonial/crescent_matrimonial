...
function getStateOfResidence(person: PersonWithDetails): string | null {
  const field = getFieldFromSection(person, 'about', /state.*reside|state.*live/i);
  if (field && field.answer.trim()) return field.answer.trim();
  return null;
}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs text-slate-500">
            {match.age && `${match.age} • `}
            {getCountryOfEthnicity(match) !== '—' && `${getCountryOfEthnicity(match)} • `}
            {getStateOfResidence(match) ?? match.location ?? '—'}
          </p>
        </div>
...
export function MatchmakingEngine({ initial, onClearInitial }: { initial: EngineInitial | null; onClearInitial: () => void }) {
  ...
  const [showManualCloseMatches, setShowManualCloseMatches] = useState(false);
  ...
  const manualCloseMatches = useMemo(() => {
    if (!showManualCloseMatches || !selectedPerson) return [];
    ...
    const candidates = people.filter(
      (p) =>
        ...
        p.gender === oppositeGender &&
        !p.is_deleted &&
        !blocked.has(pairKey(selectedPerson.id, p.id)),
    );
    const scored: CloseMatchEntry[] = [];
    for (const c of candidates) {
      const r = computeCompatibility(selectedPerson, c);
      if (r.score >= 80) {
    ...
    return scored;
  }, [showManualCloseMatches, selectedPerson, people, oppositeGender, matches, dismissedPairs]);
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="truncate text-xs text-slate-500">
                          ...
                          {getCountryOfEthnicity(partner) !== '—' && `${getCountryOfEthnicity(partner)} • `}
                          {getStateOfResidence(partner) ?? partner.location ?? '—'}
                        </p>
                      </div>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  onClick={() => {
                    ...
                    setShowManualCloseMatches(false);
                  }}
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="truncate text-xs text-slate-500">
                                {other.age && `${other.age} • `}
                                {getCountryOfEthnicity(other) !== '—' && `${getCountryOfEthnicity(other)} • `}
                                {getStateOfResidence(other) ?? other.location ?? '—'}
                              </p>
                            </div>
                            <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/15 px-2 py-0.5 text-xs text-emerald-300 ring-1 ring-emerald-500/30">
                              <HeartHandshake className="h-3 w-3" /> In Active Pair
            ...
            {!manualSelected ? (
              <>
                ...
                {!manualQuery.trim() && (
                  <div className="mt-3">
                    <button
                      onClick={() => setShowManualCloseMatches((v) => !v)}
                ...
                <div className="flex items-center gap-3 rounded-lg border border-slate-700/50 bg-slate-950/40 px-3 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-slate-500">
                      ...
                      {getCountryOfEthnicity(manualSelected) !== '—' && `${getCountryOfEthnicity(manualSelected)} • `}
                      {getStateOfResidence(manualSelected) ?? manualSelected.location ?? 'Location unknown'}
                    </p>
                  </div>
