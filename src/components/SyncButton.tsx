...
export function SyncButton() {
  const { syncFromSheets, useMock } = useData();
  ...
  const sync = async () => {
    ...
    setMessage('');
    try {
      const total = await syncFromSheets();
      setCount(total);
      setState('done');
      setMessage(
        total > 0
          ? `Synced ${total} new candidates from Google Sheets.`
          : 'All sheet candidates are already in the database.',
      );
